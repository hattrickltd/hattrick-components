...
export class Bar {
  ...
  render() {
    if (!this.didLoad) return (<div></div>);

    let r = [
      <div class={{ "bar-level": true, "maxed": this.isCap }} style={{ "width": this.skillWidth + "%" }}></div>
    ];
    if (this.capWidth > 0) r.push(
      <div class="bar-cap" style={{ "width": this.capWidth + "%" }}></div>
    );
    if (this.skillWidth + this.capWidth < 100) r.push(
