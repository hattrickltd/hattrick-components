...
| [Match clock](src/components/match-clock/) | A match clock which counts down during halftime and overtime breaks, also counts down if the match hasn't started yet. |
...
| [Timer](src/components/timer/) | A simple timer to display hh:mm:ss, optionally it can also start counting upwards if the deadline has passed. |
